...
(function (window, $, document) {   
    ...
    $.fn.toExcelUTCTime = function (date1){
    ...
    $.fn.toExcelLocalTime = function (date1){
		var d2=Math.floor(date1.getTime()/1000);													// Number of seconds since JS epoch
		d2=Math.floor(d2/86400)+25569;																// Days since epoch plus difference in days between Excel EPOCH and JS epoch
		var seconds = date1.getUTCSeconds()+60*date1.getUTCMinutes()+60*60*date1.getUTCHours();		// Number of seconds of received hour
    ...
    for (var i = 0; i < $.fn.borderStyles.length; i++) borderStylesUpper.push($.fn.borderStyles[i].toUpperCase());
    ...
    function toBorderXml(b) {
        ...
        for (var i = 0; i < 4; i++) {
            ...
            if (vals[0] == "NONE") s = s + "/>";
                ...
                var border = $.fn.borderStyles[borderStylesUpper.indexOf(vals[0])];
                if (border)
    ...
    $.fn.new = function(defaultFont) {
        ...
        excel.generate = function (filename) {
            ...
            console.log(sheets.toRels());                                                       // Add WorkBook RELS   
            ...
            zip.generateAsync({ type: "blob",mimeType:"application/vnd.ms-excel" })
                .then(function (content) { saveAs(content, filename); });        // And generate !!!
